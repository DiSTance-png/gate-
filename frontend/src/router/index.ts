...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'incomplete-decisions', name: 'admin-incomplete-decisions', component: () => import('../views/admin/IncompleteDecisionsPage.vue') },
...
router.afterEach((to) => {
  ...
  let title = 'Gate Quantum Trader | Gate Futures 量化终端'
  let isNoIndex = false

  if (to.path === '/docs' || to.path.startsWith('/docs/')) {
    title = '开发与使用指南 | Gate Quantum Trader 文档中心'
  } else if (to.path === '/factors') {
    title = '多因子动能矩阵 | Gate Quantum Trader'
  } else if (to.path === '/news') {
    title = '全网舆情与聪明钱雷达 | Gate Quantum Trader'
  } else if (to.path === '/lab') {
    title = 'AI 策略自进化认知中枢 | Gate Quantum Trader'
  } else if (to.path === '/history') {
    title = '交易台账与复盘审计 | Gate Quantum Trader'
  } else if (to.path.startsWith('/admin')) {
    isNoIndex = true
    const adminLabels: Record<string, string> = {
      'admin-overview': '运行总览',
      'admin-security': 'Gate 账户与合约池',
      'admin-council': '模型委员会',
      'admin-policy': '策略版本快照',
      'admin-llm': '模型连接与供应商',
      'admin-notify': '消息通知中心',
