...
const allProtected = computed(() =>
  store.positions.length > 0 && store.positions.every((p: any) => p.protectionStatus === 'fully_protected' && p.cloud_oco_verified === true)
)
</script>
        ...
        <tbody class="divide-y" style="border-color: var(--border-subtle);">
          ...
          >
            ...
            <td class="py-3 px-3.5">
              ...
              >
                <ShieldCheck v-if="pos.displayStop" class="w-3 h-3 shrink-0" />
                <ShieldAlert v-else class="w-3 h-3 shrink-0" />
                <span class="font-bold num-tabular">{{ pos.displayStop ? `$${pos.displayStop}` : '未设置' }}</span>
              </div>
            </td>
