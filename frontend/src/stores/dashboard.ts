...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const factors = computed<InstrumentFactor[]>(() => {
    ...
    for (const li of libInstruments) {
      const key = String(li?.name || li?.instId || '').replace('-USDT-SWAP', '_USDT')
      if (key) libMap.set(key, li)
    }
    return rawFactors.map((f: any) => {
      const normalizedId = String(f.instId || '').replace('-USDT-SWAP', '_USDT')
      const lib = libMap.get(normalizedId) || {}
      const calc = lib.calculus_dynamics || lib.calculus || f.calculus || {}
      ...
      return {
        ...
        calculus: {
          velocity_1h: calc.velocity_1h ?? calc.velocity,
          accel_1h: calc.accel_1h ?? calc.acceleration,
          jerk_1h: calc.jerk_1h ?? calc.jerk ?? calc.max_abs_jerk,
          impulse_1h: calc.impulse_1h ?? calc.impulse,
          energy_1h: calc.energy_1h ?? (calc.definite_integrals || lib.definite_integrals || {}).energy_integral,
          action_area_1h: calc.action_area_1h ?? (calc.definite_integrals || lib.definite_integrals || {}).deviation_area_integral,
          state_1h: calc.state_1h ?? calc.regime,
        },
        smart_money: {
  ...
  // Keep the chart's existing factor-library contract while sourcing Gate-native data.
  // The chart expects the legacy camelCase fields (for example `atr1h`).
  const factorLibrary = computed<any[]>(() => {
    const instruments: any[] = (data.value as any)?.factor_library?.instruments || []
    return instruments.map((item: any) => ({
      ...item,
      instId: item.instId || item.name,
      atr1h: item.atr1h ?? item.atr_1h ?? item.atr,
      c_1h_ret: item.c_1h_ret ?? (Number(item.chg24h || 0) / 100),
    }))
  })
