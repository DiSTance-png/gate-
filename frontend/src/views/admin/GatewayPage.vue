...
async function activate() {
  try {
    const res = await api('/api/v1/admin/gateway/activate', { method: 'POST', body: '{}' })
    bannerMsg.value = { text: res.message || 'Gateway 激活请求已发送', type: 'ok' }
    setTimeout(load, 800)
  } catch (e: any) {
  ...
  <div class="space-y-4">
    ...
    <template v-else-if="gw">
      ...
      <div class="flex justify-end gap-2">
        <button @click="load" class="flex items-center space-x-1 px-2.5 py-1 rounded-lg border text-[11px] font-mono cursor-pointer" style="background-color: var(--bg-card); border-color: var(--border-medium); color: var(--text-main);"><RefreshCw class="w-3 h-3" /><span>刷新状态</span></button>
        <button v-if="!gw.running" @click="activate" class="flex items-center space-x-1 px-2.5 py-1 rounded-lg border text-[11px] font-mono font-bold cursor-pointer" style="background-color: var(--color-up-bg); border-color: var(--color-up-border); color: var(--color-up);"><Zap class="w-3 h-3" /><span>激活 Gateway</span></button>
      </div>
