...
# Windows consoles often default to GBK; strategy diagnostics contain Chinese
# and symbols, so never let a final log print abort an otherwise completed run.
...
from okx_runtime import freeze_environment as freeze_okx_environment, replace_cli_prefix as okx_private_command, unfreeze_environment as unfreeze_okx_environment, selected_environment, okx_urlopen
import json
import secrets
import time
import datetime
...
from r20_backend.okx_trade_service import _request as okx_v5_request
...
def clean_stale_open_orders() -> Tuple[bool, str]:
    ...
    try:
        orders = okx_v5_request("GET", "/api/v5/trade/orders-pending", {"instType": "SWAP"}, timeout=20)
    except Exception as exc:
        # Demo mode is allowed to keep producing research decisions when the
        # optional OKX private proxy is temporarily unavailable. Live mode must
        # remain fail-closed because order state cannot be safely inferred.
        if selected_environment().simulated:
            print(f"[Trader] Demo fallback: unable to verify open orders; continuing without cleanup: {exc}")
            return True, "demo fallback: open orders unavailable"
    ...
    for order in orders:
        ...
        if state not in {"live", "partially_filled"} or not order_id or now_ts - created_at <= 240000:
            continue
        try:
            okx_v5_request("POST", "/api/v5/trade/cancel-order", {"instId": inst_id, "ordId": order_id}, timeout=20)
        except Exception as exc:
...
def query_positions() -> Tuple[bool, List[Dict[str, Any]], str]:
    ...
    try:
        return True, okx_v5_request("GET", "/api/v5/account/positions", {"instType": "SWAP"}, timeout=20), ""
    except Exception as exc:
        if selected_environment().simulated:
            print(f"[Trader] Demo fallback: unable to verify positions; assuming no open positions: {exc}")
            return True, [], "demo fallback: positions unavailable"
        return False, [], str(exc)
...
def submit_protected_limit_order(inst_id: str, side: str, pos_side: str, size: int, price: float, tp_px: float, sl_px: float) -> Tuple[bool, str]:
    ...
    client_order_id = f"R20{int(time.time() * 1000)}{secrets.token_hex(3)}"
    order_params = {
        "instId": inst_id,
        "tdMode": "cross",
        "side": side,
        ...
        # OKX V5 now requires entry-order TP/SL protection in the
        # attachAlgoOrds array. The legacy top-level fields return the
        # opaque business error `1: All operations failed` on demo.
        "attachAlgoOrds": [{
            "tpTriggerPx": str(effective_tp),
            "tpOrdPx": "-1",
            "slTriggerPx": str(effective_sl),
            "slOrdPx": "-1",
        }],
    }

    def lookup_ambiguous_order() -> Dict[str, Any]:
        try:
            rows = okx_v5_request(
                "GET", "/api/v5/trade/order",
                {"instId": inst_id, "clOrdId": client_order_id}, timeout=12,
            )
    ...
    for submit_attempt in range(2):
        try:
            payload_rows = okx_v5_request("POST", "/api/v5/trade/order", order_params, timeout=20)
            payload = payload_rows[0] if payload_rows else {}
            break
        except Exception as exc:
            error = str(exc)
            if "网络请求失败" not in error:
                return False, error
            first_error = first_error or error
            payload = lookup_ambiguous_order()
            if payload:
                break
            if submit_attempt == 0:
                time.sleep(1)
            ...
            payload = lookup_ambiguous_order()
            if not payload:
                return False, f"{first_error}; 已使用 clOrdId 复查，OKX 未确认接单"
    order_id = None
...
def fetch_single_instrument_data(item, all_positions, usdt_available):
    ...
    if raw_15m:
        ...
        try:
            ...
            with okx_urlopen(req_t, timeout=3) as response_t:
...
def single_trader_cycle(func):
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            if not acquire_lock(lock_handle, non_blocking=True): raise BlockingIOError
...
def execute_portfolio():
    ...
    try:
        pending_orders = okx_v5_request("GET", "/api/v5/trade/orders-pending", {"instType": "SWAP"}, timeout=20)
    except Exception as exc:
        if selected_environment().simulated:
            print(f"[Trader] Demo fallback: unable to verify pending orders; assuming none: {exc}")
            pending_orders = []
        else:
    ...
    try:
        balance_rows = okx_v5_request("GET", "/api/v5/account/balance", {"ccy": "USDT"}, timeout=20)
        bal_res = [{"details": balance_rows[0].get("details", [])}] if balance_rows else []
    except Exception as exc:
        if selected_environment().simulated:
            print(f"[Trader] Demo fallback: unable to verify account balance; using zero available balance: {exc}")
            bal_res = []
        else:
