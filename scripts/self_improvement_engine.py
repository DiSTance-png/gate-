...
def run_self_evolution(force: bool = False):
    ...
    try:
        ...
        archive_name = f"{timestamp_str.replace(':', '').replace(' ', '_')}_{ledger_revision[:12]}.json"
        atomic_write_json(os.path.join(REPORT_ARCHIVE_DIR, archive_name), report_payload)
    except Exception as exc:
        log_msg(f"Failed to archive self-evolution report: {exc}")
