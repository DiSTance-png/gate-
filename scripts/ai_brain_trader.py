...
from okx_runtime import replace_cli_prefix as okx_private_command, okx_urlopen
import json
import time
import datetime
...
def fetch_single_instrument_package(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item["type"] == "crypto":
        try:
            funding_rate = fetch_funding_rate(inst_id)
            if funding_rate is not None:
                pkg["fundingRate"] = funding_rate
        except Exception:
            pass
        ...
        if ccy:
            try:
                ...
                with okx_urlopen(req, timeout=3) as resp:
    ...
    try:
        ...
        if brain_output is None:
            ...
            if execute_llm_request:
                content, reasoning_content, usage_dict, _ = execute_llm_request(
                    messages=[
            ...
            # Some reasoning-capable gateways place the final JSON in the
            # reasoning channel and leave message.content empty. Recover the
            # object before treating the inference as failed.
            content = (content or "").strip()
            if not content and reasoning_content:
                import re
                candidates = re.findall(r"\{.*\}", reasoning_content, flags=re.DOTALL)
                if candidates:
