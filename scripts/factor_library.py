...
from okx_runtime import okx_urlopen

WORKSPACE_DIR = str(_PROJECT_ROOT)
DATA_DIR = os.path.join(WORKSPACE_DIR, "data")
FACTOR_LIB_CACHE_FILE = os.path.join(DATA_DIR, "factor_library_snapshot.json")

os.environ["PATH"] = os.pathsep.join([os.path.join(os.path.expanduser("~"), "AppData", "Local", "npm"), os.environ.get("PATH", "")])
...
def compute_instrument_factors(item: Dict[str, Any], smart_money_pool: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if funding_rate is not None:
            factors["smart_money_derivatives"]["funding_rate_pct"] = funding_rate
    except Exception:
        pass
