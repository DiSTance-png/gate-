...
class OKXEnvironment:
    ...
    def cli_env(self, base: Mapping[str, str] | None = None) -> dict[str, str]:
        ...
        if self.proxy_url:
            env["R20_OKX_PROXY"] = self.proxy_url
        return env

    def cli_prefix(self) -> str:
        wrapper = ROOT / "bin" / "okx.cmd"
        executable = f'"{wrapper}"' if os.name == "nt" and wrapper.exists() else "okx"
        return f"{executable} --{self.mode}"
...
def selected_environment(values: Mapping[str, str] | None = None) -> OKXEnvironment:
    ...
    proxy_url = str(env.get("R20_OKX_PROXY") or "").strip()
    if base_url != "https://www.okx.com": raise ValueError("OKX REST Base URL 只允许 https://www.okx.com")
    return OKXEnvironment(mode, api_key, secret_key, passphrase, base_url, "separate-credentials" if env.get(f"{prefix}_API_KEY") else "legacy-or-oauth", proxy_url)
...
def replace_cli_prefix(command: str, values: Mapping[str, str] | None = None) -> str:
    ...
    if selected.proxy_url:
        os.environ["R20_OKX_PROXY"] = selected.proxy_url
...
def okx_urlopen(request, timeout: float = 10, values: Mapping[str, str] | None = None):
    """Open an OKX HTTP request through the configured VPS proxy."""
    selected = _FROZEN_ENVIRONMENT or selected_environment(values)
