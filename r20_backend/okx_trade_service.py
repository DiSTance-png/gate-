...
def _run_cli(command: list[str], timeout: int = 20, env: OKXEnvironment | None = None) -> list[dict[str, Any]]:
    try:
        ...
        cli_env = selected.cli_env()
        if selected.proxy_url:
            cli_env.update({"HTTP_PROXY": selected.proxy_url, "HTTPS_PROXY": selected.proxy_url, "ALL_PROXY": selected.proxy_url})
        res = subprocess.run(command, capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=timeout, env=cli_env)
...
def _request(method: str, path: str, params: dict[str, Any] | None = None, env: OKXEnvironment | None = None, timeout: int = 20) -> list[dict[str, Any]]:
    # Gate is selected explicitly and stays isolated from the OKX credential path.
    # The adapter returns the normalized OKX-shaped rows expected by legacy strategies.
    from .gate_runtime import selected_exchange
    if selected_exchange() == "gate":
        from .gate_client import GateClient
        return GateClient().request_okx_compat(method, path, params)
    selected = env or selected_environment()
    if not selected.configured:
        ...
        if path == "/api/v5/account/positions":
            ...
            return _run_cli(cmd, timeout=timeout, env=selected)
    ...
    for attempt in range(attempts):
        ...
        try:
            opener = urllib.request.build_opener(urllib.request.ProxyHandler({"http": selected.proxy_url, "https": selected.proxy_url})) if selected.proxy_url else urllib.request.build_opener()
            with opener.open(request, timeout=timeout) as response:
                payload = json.loads(response.read().decode("utf-8") or "{}")
            break
        except Exception as exc:
            if attempt + 1 >= attempts:
                raise RuntimeError(f"OKX V5 网络请求失败（尝试 {attempts} 次）：{type(exc).__name__}: {exc}") from exc
    ...
    if failures:
        failure = failures[0]
        code = failure.get("sCode") or "--"
        message = failure.get("sMsg") or failure.get("msg") or "业务请求失败"
        # Demo trading sometimes returns the unhelpful `1: All operations failed`.
        # Preserve the complete row so callers can diagnose the rejected field.
        if str(code) == "1" and message == "All operations failed":
            details = json.dumps(failure, ensure_ascii=False, separators=(",", ":"))
            raise RuntimeError(f"OKX {code}: {message}; response={details}")
        raise RuntimeError(f"OKX {code}: {message}")
