...
from r20_gateway.okx_proxy_supervisor import start_supervisor as start_okx_proxy_supervisor, stop_supervisor as stop_okx_proxy_supervisor
...
from scripts.okx_runtime import okx_urlopen
...
async def lifespan(_: FastAPI):
    ...
    gateway_enabled = os.getenv("R20_GATEWAY_WORKER_ENABLED", "1").strip().lower() in {
        "1", "true", "yes", "on",
    }
    gate_mode = settings.exchange == "gate"
    if not gate_mode:
        start_okx_proxy_supervisor()
    if gateway_enabled:
...
def get_admin_configuration() -> dict[str, str]:
    ...
    exchange_configuration = {
        "Gate 当前环境": settings.gate_environment.upper(),
        "Gate Live 凭证": "已配置" if os.getenv("GATE_LIVE_API_KEY") and os.getenv("GATE_LIVE_API_SECRET") else "未配置",
        "Gate Testnet 凭证": "已配置" if os.getenv("GATE_TESTNET_API_KEY") and os.getenv("GATE_TESTNET_API_SECRET") else "未配置",
        "Gate Live 交易开关": "显式启用" if settings.gate_live_trading_enabled else "关闭 (FAIL-CLOSED)",
        "Gate VPS 独立代理": "已配置" if settings.gate_proxy else "未配置",
    } if settings.exchange == "gate" else {
    ...
    return {
        **exchange_configuration,
        ...
        "原生保护单覆盖": "Gate price_orders 覆盖校验",
        "初始本金基准": f"{baseline.get('initial_capital', 0.0):,.2f} USDT",
...
def runtime_overview() -> dict[str, Any]:
    ...
    # Gate uses the exchange-side Futures account_book for PnL/fees/funding;
    # the legacy local trading_ledger.json is an OKX-era artifact and must not
    # make Gate data health stale.
    if settings.exchange != "gate":
        health_files.append(file_health("trading_ledger.json", 15 * 60))
    ...
    gate_credentials = bool(os.getenv("GATE_TESTNET_API_KEY") and os.getenv("GATE_TESTNET_API_SECRET")) if settings.gate_environment == "testnet" else bool(os.getenv("GATE_LIVE_API_KEY") and os.getenv("GATE_LIVE_API_SECRET"))
    return {
        "service": {"version": __version__, "pid": os.getpid(), "uptime_seconds": int(time.time() - STARTED_AT)},
        "exchange": settings.exchange,
        "environment": settings.gate_environment if settings.exchange == "gate" else settings.okx_environment,
        "credentials": {"gate": gate_credentials, "llm": bool(settings.llm_api_key)},
        ...
        "logs": {
            "trader": log_tail("r20_gateway.log", 18),
            "backend": log_tail("r20_gateway.log", 18),
            "scheduler": log_tail("r20_gateway.log", 18),
        },
...
ADMIN_LOG_SOURCES = {
    "trader": "gate_trader.log",
    "backend": "gate_backend.log",
...
def admin_runtime(x_r20_admin_token: str | None = Header(default=None), x_r20_session: str | None = Header(default=None, alias="X-R20-Session")) -> dict[str, Any]:
    ...
    history = read_json("ai_decision_history.json", [])
    if not isinstance(history, list):
        history = []
    if not history and isinstance(raw_decisions, dict):
        current_decisions = {}
        for symbol, envelope in raw_decisions.items():
            if not isinstance(envelope, dict) or not isinstance(envelope.get("decision"), dict):
                continue
            decision = envelope["decision"]
            current_decisions[symbol] = {
                "action": decision.get("action", "WAIT"),
                "confidence": decision.get("confidence", 0),
                "summary_reason": decision.get("summary_reason", ""),
                "rejection_reason": decision.get("rejection_reason", ""),
            }
        if current_decisions:
            history = [{
                "generated_at_ms": raw_decisions.get("generated_at_ms", 0),
                "environment": raw_decisions.get("environment", "gate"),
                "trade": raw_decisions.get("trade", {}),
                "decisions": current_decisions,
            }]
    payload["decision_history"] = list(reversed(history[-3:]))
...
def gateway_status(x_r20_admin_token: str | None = Header(default=None), limit: int = 50) -> dict[str, Any]:
    ...
    # The supervisor owns PID validation. This also recovers when the worker
    # was started before the control plane wrote its PID file.
    ...
    if pid:
        try:
            ...
            pass
    running = running or (not pid and _worker_lock_held())
...
def activate_gateway(x_r20_admin_token: str | None = Header(default=None), x_r20_session: str | None = Header(default=None, alias="X-R20-Session")) -> dict[str, Any]:
    """Explicitly activate the Gate-owned Gateway supervisor and worker."""
    refresh_settings()
    actor = require_admin_header(x_r20_admin_token, x_r20_session)
    if settings.exchange != "gate":
        raise HTTPException(status_code=409, detail="当前不是 Gate 模式，拒绝从 Gate 控制面启动 Gateway")
    start_gateway_supervisor()
    from r20_gateway.supervisor import current_pid
    pid = current_pid()
    audit_record("gateway.activate", "accepted", {"actor": actor.get("username", "admin"), "pid": pid or None})
    return {"ok": True, "running": bool(pid), "pid": pid or None, "message": "Gate Gateway 已请求激活；请刷新状态确认 worker 已接管"}
...
def run_gateway_job(
    ...
    gate_modules = {
        "trader": "gate_quant.ai_worker",
        "factor_library": "gate_quant.factor_worker",
        "news": "gate_quant.news_worker",
    }
    if job_id in gate_modules:
        cmd = [sys.executable, "-m", gate_modules[job_id], *job_cfg["args"]]
    else:
...
def update_admin_config(payload: AdminConfigUpdate, x_r20_admin_token: str | None = Header(default=None), x_r20_session: str | None = Header(default=None, alias="X-R20-Session")) -> dict[str, Any]:
    ...
    else: require_admin_header(x_r20_admin_token, x_r20_session)
...
def market_candles(inst_id: str, bar: str = "1H", limit: int = 150, response: Response = None) -> dict[str, Any]:
    ...
    try:
        ...
        with okx_urlopen(req, timeout=5) as resp:
...
# The Gate outer application serves API routes before mounting this control plane.
# Keep Vue history-mode admin URLs working without importing the legacy dashboard,
# whose module starts an OKX-oriented background worker at import time.
@app.get("/admin", include_in_schema=False)
@app.get("/admin/{path:path}", include_in_schema=False)
def gate_admin_spa(path: str = ""):
    index = VUE_DIST / "index.html"
    if not index.exists():
        raise HTTPException(status_code=503, detail="Gate frontend has not been built")
