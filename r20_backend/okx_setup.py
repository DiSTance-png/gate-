...
def _effective_path(path: str | None = None) -> str:
    """Include the per-user npm bin directory on native Windows."""
    value = path if path is not None else os.environ.get("PATH", "")
    if os.name == "nt":
        candidates = [
            str(Path.home() / "AppData" / "Local" / "npm"),
            os.environ.get("ProgramFiles", "") + "\\nodejs",
        ]
        parts = [item for item in value.split(os.pathsep) if item]
...
def _effective_env(env: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    result["PATH"] = _effective_path(result.get("PATH"))
    proxy = result.get("R20_OKX_PROXY", "")
    if not proxy:
        env_file = Path(__file__).resolve().parents[1] / ".env"
        if env_file.exists():
            for line in env_file.read_text(encoding="utf-8").splitlines():
                if line.strip().startswith("R20_OKX_PROXY="):
                    proxy = line.split("=", 1)[1].strip().strip('"').strip("'")
                    break
    if proxy:
        result.update({"R20_OKX_PROXY": proxy, "HTTP_PROXY": proxy, "HTTPS_PROXY": proxy, "ALL_PROXY": proxy})
    return result
...
def check_node_npm() -> dict[str, Any]:
    ...
    okx = shutil.which("okx", path=path)
    okx_version = ""
    if okx:
...
def install_okx_cli() -> dict[str, Any]:
    ...
    existing_binary = shutil.which("okx", path=_effective_path())
    existing_version = ""
    if existing_binary:
    ...
    # Verify post-install
    binary = shutil.which("okx", path=_effective_path())
    if not binary:
        return {
            "ok": False,
...
def _run(command: list[str], timeout: int = 12, env: Mapping[str, str] | None = None) -> dict[str, Any]:
    try:
        # OKX CLI emits UTF-8 even when Windows' active code page is GBK.
        # Decode explicitly so OAuth/install diagnostics do not become HTTP 500.
...
def start_oauth_device_login(site: str, *, force_relogin: bool = False, env: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    binary = shutil.which("okx", path=_effective_path((env or os.environ).get("PATH")))
    if not binary:
