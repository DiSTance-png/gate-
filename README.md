...
后台“异常与未闭环”执行独立只读审计，不再只检查 Trader 返回码。它会继续核对六合约决策完整性、持仓管理结论、Gateway/Trader/对账器心跳、执行台账错误、成交与保护数量、实时 TP/SL 覆盖、过期与孤立订单，以及安全门快照和实时 Gate 状态是否一致。异常按环境保存首次发现、最后出现、累计次数和恢复时间到 `data/gate_anomaly_history.json`；检测器不会自动重跑 AI、补单、撤单或平仓。
