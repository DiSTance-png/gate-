...
默认启用 Gate 专用 SSH 隧道：本地 `127.0.0.1:18081` 经 SSH 主机别名 `my-vps` 转发到 VPS 的 `127.0.0.1:8888` tinyproxy。`GATE_REQUIRE_PROXY=true` 禁止 Gate 请求回退本地直连；隧道创建失败时 Gate 服务启动失败。OKX 的本地 `18080` 隧道不被读取或修改。
