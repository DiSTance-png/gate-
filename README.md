...
当前独立版本：`Gate v1.0.0`。版本线不再沿用上游 R20 的 `v7.x` 编号，前端视觉暂时保持现状。
...
后台“Gate 账户与合约池”可选择 `GATE_RISK_PROFILE`、修改 `GATE_LEVERAGE` 和单轮最大开仓数量。保守、标准、积极档每轮最多 1 单；激进档可选 1 或 2 单；观察模式为 0。保存后，后端生成带版本和哈希的 `gate-risk-v2` 风险快照；同一份快照生成本周期唯一风险预算，并控制置信度/ADX/R:R、有效保证金额度、止损 ATR 以及分档锁盈提示。继承模板中的旧固定金额、杠杆、置信度和锁盈数字会在 Gate 适配边界被移除。AI 输出的 leverage 不能覆盖系统配置；真正下单前仍调用 Gate 原生持仓杠杆接口。多候选按置信度串行执行，每一笔前重新读取账户和持仓；单个币种等待报价确认或被拒绝不会遮挡后续合格币种，只有订单状态不明确才停止整轮。
...
后台“异常与未闭环”每 60 秒执行独立只读审计，不再只检查 Trader 返回码。它会核对全部 Gateway 任务、六合约决策完整性、持仓管理结论、Trader/对账器心跳、执行台账错误、成交与保护数量、实时 TP/SL 覆盖、过期与孤立订单，以及安全门快照和实时 Gate 状态是否一致。当前/已恢复状态保存在 `data/gate_anomaly_history.json`，每次发现、持续和恢复事件不可变追加到 `data/gate_anomaly_history_events.json`；页面默认显示“最近全部”，也可切换当前异常和已恢复。检测器不会自动重跑 AI、补单、撤单或平仓。

首页“平仓笔数”来自 Gate `/position_close` 聚合仓位退出，“Gate 结算”来自 `account_book` 的原始 PnL 事件；净盈亏继续按 `pnl + fee + fund` 汇总。因此一次分批平仓可以对应一笔平仓和多条结算事件，二者不再混为同一数字。
...
自动复盘默认只写入 `data/evolution_candidates/` 候选，不会直接覆盖当前稳定心法或标的倍率。超级管理员可在“自进化配置”审核应用或拒绝候选。收益快照基于真实平仓台账统计净盈亏、手续费、最大回撤和分标的表现；台账没有资金费或滑点字段时显示“不可用”，不会估算或伪造。Gate 合约名会按当前标的池动态归一化，自进化页面展示原始/接受/过滤样本数、过滤原因和快照匹配情况。可用 `GATE_EVOLUTION_START_TIME=YYYY-MM-DD HH:MM:SS` 独立设置复盘起点；成交后 20 分钟内首次巡检写入的同方向信号快照允许匹配，未来超过 20 分钟或过期超过 6 小时的快照不会作为开仓证据。

Gateway 启动时会把上一次异常终止遗留的 `running` 任务标为“已中断”，并默认保留 30 天任务历史；可用 `GATE_JOB_RUNS_KEEP_DAYS` 调整，但不能小于 1 天。

止损生命周期使用已验证的安全替换流程：先创建并确认新的 Gate 原生保护单，再撤销旧保护单；创建或确认失败时保留旧保护，不使用未经 Gate 官方文档和 Testnet 验证的计划单改单接口。
