...
- “没有仓位”可能是限价单未成交、已撤单、触发止损、AI 复核退出或达到绝对持仓上限，必须结合订单和 Gate 平仓台账判断。
...
- 显式入场意图和突破计划默认关闭；虽已完成 Gate Testnet 正常链验证，仍不得据此自动在 Live 启用，Live 必须由用户另行显式决定。
...
- 修改前基线提交：`b062f45`。
- 离线测试：126 项通过。
- 已在 Gate Testnet 正常链验证行情、账户、持仓、挂单、成交、撤单、保护单、过期撤单、原生平仓台账，以及突破计划多空触发、`trade_id` 对账、真实成交价保护重算和清理归零。双向持仓原生平仓与“复核阈值 + 绝对上限”已完成离线验证，尚待下一次 Testnet 回归。
