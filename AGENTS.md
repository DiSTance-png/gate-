...
同轮候选必须串行执行。保守、标准、积极档最多实际提交 1 单，激进档按 `GATE_MAX_ENTRIES_PER_CYCLE` 可提交 1 或 2 单。等待二次报价确认、冷却或确定性风控拒绝只影响当前合约，不得遮挡后续合格候选；订单状态不明确时必须停止整轮。第二笔前必须刷新账户和持仓，并计入第一笔已提交但交易所字段可能尚未反映的保证金与名义敞口预留。
