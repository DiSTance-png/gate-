...
6. 调用配置的 OpenAI-compatible AI 服务，要求一次返回六合约决策及持仓/挂单管理建议；显式入场意图启用时，每个非观望决策还必须返回 `entry_intent`。
7. 将 AI 返回值规范化，置信度强制限制在 `0-100`，杠杆强制替换为系统配置；缺失或非法的入场意图按合约转为观望。
8. 再经过确定性策略拦截器检查：数据质量、置信度、ADX、4H 方向、止盈止损几何、R:R 和持仓方向。
9. 从通过的候选中选取最高置信度标的。
10. 再检查入场意图与买一/卖一关系、冷却、安全门、最小张数、仓位名义价值、单笔保证金和总保证金。
11. 全部通过后才进入下单状态机。
...
- `GATE_MAX_ENTRIES_PER_CYCLE` 控制单轮实际提交订单数量；观察模式为 0，保守/标准/积极最多 1，激进最多 2。
- 多候选按置信度串行处理。某币种等待下一轮报价确认、处于冷却或被确定性风控拒绝时继续检查后续候选，不占实际提交名额。
- 第二笔下单前重新读取账户与持仓，并保守计入第一笔尚未及时反映在 Gate 账户字段中的风险预留；订单状态不明确时整轮 fail-closed。
...
- `immediate`：报价必须接近当前买一/卖一，使用带最大滑点的 IOC 限价。
- `retracement`：做多价低于卖一、做空价高于买一，使用 GTC 限价。
- `breakout`：做多触发价高于卖一、做空触发价低于买一，使用 Gate 原生 `price_orders`。Gate 当前要求原生有效期为 86400 秒的整数倍，系统固定提交 86400 秒，并由 10 秒对账器按本地 840 秒生命周期提前撤销和确认。

执行器不根据价格位置猜测意图。字段缺失、取值非法或价格关系矛盾时只拒绝该合约。滑点与有效期来自系统配置，AI 不能覆盖。
...
Gate 官方 `FuturesInitialOrder.size` 对 `price_orders` 定义为整数张。突破计划因此在普通张数计算后再次向零取整，绝不向上放大风险；不足一张时安全拒绝。等待触发的计划单虽然尚未形成仓位，仍按触发价预留潜在保证金和名义敞口。
...
| `awaiting_trigger` | Gate 突破计划已确认存在，等待上破/下破 |
| `triggered_order_pending` | 计划已触发，等待通过 `trade_id` 确认真实订单 |
| `pending_fill` | 订单存在但尚未成交 |
| `position_pending` | 订单显示成交，等待持仓接口最终一致 |
| `partially_filled` | 部分成交且已按成交量保护，继续等待余量 |
| `stop_protected_tp_pending` | 止盈失败后，本次新增止损撤销状态不明；保留止损并安全暂停 |
| `protection_pending` | 保护覆盖尚未完整确认 |
| `manual_review` | 状态无法自动确认，需要人工检查 |

主要终态包括 `trigger_expired`、`trigger_cancelled`、`trigger_failed`、`unfilled`、`abandoned_unconfirmed`、`filled_protected`、`order_rejected`、`stop_failed_flatten_attempted`、`take_profit_failed_flatten_attempted` 和 `flattened_invalid_protection`。止损或止盈创建失败时，系统仅按当前仓位相对下单前基准仓位的新增量回滚；止盈失败会先确认撤销本次新增止损，撤销结果不明确时不贸然减仓。

活动状态存在时，新增风险通常关闭。已确认仍在交易所等待触发的 `awaiting_trigger` 是唯一例外：它只占对应合约并预留组合风险，不阻止其他合约；触发、撤单或查询状态不明确时立即回到组合级 fail-closed。Gate 返回但本地执行台账无法匹配的非减仓计划单会记录为 `untracked_trigger_entry_orders` 并关闭安全门，不会被误归类为孤立保护单。不能通过删除 SQLite 数据库解除安全门。
...
突破计划 POST 超时先按 `initial.text` 在 open/finished `price_orders` 中查找；找到后接管，找不到则进入人工复核，不重复提交。触发后根据 Gate 返回的 `trade_id` 查询真实订单。撤单超时同样重新读取 open 计划单确认，不盲目重复撤销。
...
它使用订单成交量与真实持仓增量交叉判断本次实际成交。新格式意图还保存 AI 入场价到 TP/SL 的百分比距离；取得真实平均成交价后按该距离重算并按 tick size 取整，再验证多仓 `SL < 当前价和成交价 < TP`、空仓反向关系。随后只补保护缺口：
...
- `execution_reconciliation_pending`：执行状态机有成交、触发或撤单状态尚不明确的活动意图；单纯 `awaiting_trigger` 不触发此项。
- 私有 Gate API、代理或凭证错误。
...
- Gate 原生平仓台账。
- AI 六合约决策链。
- 突破计划单创建、按客户端订单号查询、主动撤销和本地 840 秒到期撤销。
- BTC 多空突破最小张数触发、`trade_id` 对账、真实成交均价 TP/SL 重算和双保护覆盖。
- 验证后撤销保护、仅平测试仓位并确认账户持仓与订单归零。
...
- 进程重启后从 SQLite 恢复。
- 止损失败时撤余量并只减新增仓位。
- 止盈失败时先确认撤销本次新增止损，再只回滚新增仓位；撤销状态不明时保留止损并关闭安全门。
...
- 网络超时、部分成交和保护创建失败等难以稳定制造的受控 Testnet 异常写回归。
