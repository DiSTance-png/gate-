...
RISK_PROFILES: dict[str, RiskProfile] = {
    "observe": RiskProfile(
        "observe", "观察模式", "只生成和审计 AI 决策，绝不提交订单。", 70, 75, 14, 2.0, 2.2, 0.0, 3, 1.6, 2.2, False
    ),
    "conservative": RiskProfile(
        "conservative", "保守", "提高质量门槛并缩小保证金，适合验证稳定性。", 85, 88, 22, 2.4, 2.8, 0.35, 3, 2.0, 2.4
    ),
    "standard": RiskProfile(
        "standard", "标准", "平衡信号质量与机会频率，保留完整硬风控。", 80, 83, 18, 2.2, 2.5, 0.65, 5, 1.8, 2.2
    ),
    "active": RiskProfile(
        "active", "积极", "适度降低趋势和置信度门槛，但不降低绝对 2R 底线。", 75, 80, 16, 2.0, 2.3, 0.85, 8, 1.6, 2.2
    ),
    "aggressive": RiskProfile(
        "aggressive", "激进", "扩大可参与信号范围并使用完整保证金额度，绝对风控仍不可绕过。", 72, 78, 14, 2.0, 2.2, 1.0, 10, 1.6, 2.0
    ),
}
