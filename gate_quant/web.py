...
def gate_admin_runtime_overview(x_gate_session: str | None = Header(default=None, alias="X-R20-Session")):
    ...
    configuration = {"Gate 当前环境": settings.environment.upper(), "Gate API 凭证": "已配置" if settings.api_key and settings.api_secret else "未配置", "AI 风险档位": get_risk_profile(settings.risk_profile).label, "单轮最大开仓数": str(settings.max_entries_per_cycle), "Gate Testnet 自动交易": "已启用" if settings.testnet_execute_trades else "关闭", "Gate Live 交易开关": "显式启用" if settings.environment == "live" and settings.live_trading_enabled else "关闭 (FAIL-CLOSED)", "Gate VPS 独立代理": "已配置" if settings.proxy_url else "未配置", "执行杠杆": f"{settings.leverage:g}x", "显式入场意图": "已启用" if settings.entry_intent_enabled else "关闭", "突破最大滑点": f"{settings.max_entry_slippage_pct * 100:.2f}%", "突破单有效期": f"{settings.breakout_expiration_seconds} 秒", "持仓复核阈值": f"{settings.max_position_age_seconds / 3600:g} 小时", "绝对持仓上限": f"{settings.absolute_max_position_age_seconds / 3600:g} 小时", "原生保护单覆盖": "Gate price_orders 双保护校验", "总持仓名义敞口上限": f"{settings.max_position_notional_usd:.2f} USDT", "总保证金上限": f"{settings.max_total_margin_usd:.2f} USDT", "单笔保证金上限": f"{settings.max_order_margin_usd:.2f} USDT"}
    ...
    with gate_write_lock():
        ...
        try:
            ...
            for position in rows:
                ...
                result["closed_positions"].append({"contract": contract, "size_before": str(position.get("size")), "result": GateTradingService(client_instance, RiskLimits(settings.max_position_notional_usd, settings.max_total_margin_usd, settings.max_order_margin_usd)).close_position_safely(contract=contract, client_id=client_id, position_mode=str(position.get("mode") or ""))})
            remaining = []
